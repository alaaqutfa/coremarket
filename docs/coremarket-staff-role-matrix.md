...
| Delivery / Distribution | Assigned operational order view and delivery-status update using the existing order permissions | Products, suppliers, purchasing, accounting, cashbox | Delivery order amount and contact/address shown by existing order details; no cost/profit |
...
- Delivery order visibility is based on existing order permissions; row-level assignment scoping needs a separate review.
