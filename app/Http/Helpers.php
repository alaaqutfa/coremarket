    ...
    function getBaseURL()
    {
        ...
        if (function_exists('request')) {
            try {
                ...
                // Fall back to config/server values in CLI and early boot contexts.
            }
        }

        $host = $host ?: ($_SERVER['HTTP_HOST'] ?? parse_url($appUrl, PHP_URL_HOST));
        $scheme = $scheme ?: ($_SERVER['REQUEST_SCHEME'] ?? parse_url($appUrl, PHP_URL_SCHEME) ?: 'http');
...
if (! function_exists('get_product_category')) {
    ...
    {
        return ProductCategory::where('product_id', $product_id)->get();
    }
}
