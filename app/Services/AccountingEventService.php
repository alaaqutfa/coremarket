...
class AccountingEventService
{
    ...
    public function recordPurchaseReceipt(PurchaseReceiptItem $item, ?int $createdBy = null): AccountingEvent { return $this->record('purchase_receipt', 'expense', PurchaseReceiptItem::class, $item->id, ['purchase_order_id'=>$item->purchaseOrderItem?->purchase_order_id,'purchase_receipt_id'=>$item->purchase_receipt_id,'inventory_movement_id'=>$item->inventory_movement_id,'amount'=>$item->total_cost,'cost_amount'=>$item->total_cost,'occurred_at'=>$item->created_at,'created_by'=>$createdBy]); }
