<div class="aiz-sidebar-wrap">
    @php
        ...
        $coremarketInventoryOperationsEnabled = coremarket_feature_enabled('inventory_pro') || coremarket_feature_enabled('accounting_lite');
        ...
        $coremarketCanInventoryPolicy = $coremarketInventoryOperationsEnabled && ($coremarketOperationsOwner || auth()->user()?->can('inventory.stock.adjust'));
        $coremarketCanInventoryFamilies = $coremarketInventoryOperationsEnabled && ($coremarketOperationsOwner || auth()->user()?->can('inventory.families.manage'));
        ...
        $coremarketCanInventoryAdjustments = $coremarketInventoryOperationsEnabled && ($coremarketOperationsOwner || auth()->user()?->can('inventory.adjustments.view'));
        $coremarketCanStockCounts = $coremarketInventoryOperationsEnabled && ($coremarketOperationsOwner || auth()->user()?->can('inventory.stock_counts.view'));
        $coremarketBranchInventoryEnabled = app(\App\Services\CoreMarketBranchInventoryService::class)->branchInventoryEnabled();
        $coremarketCanBranchStock = $coremarketBranchInventoryEnabled && $coremarketInventoryOperationsEnabled && ($coremarketOperationsOwner || auth()->user()?->can('inventory.branch_stock.view'));
        $coremarketCanStockTransfers = $coremarketBranchInventoryEnabled && $coremarketInventoryOperationsEnabled && ($coremarketOperationsOwner || auth()->user()?->can('inventory.stock_transfers.view'));
        $coremarketSerialWarrantyEnabled = filter_var(get_setting('inventory.serial_tracking_enabled', config('coremarket.inventory.serial_tracking_enabled', false)), FILTER_VALIDATE_BOOL)
            || filter_var(get_setting('inventory.warranty_tracking_enabled', config('coremarket.inventory.warranty_tracking_enabled', false)), FILTER_VALIDATE_BOOL);
        $coremarketCanWarranty = $coremarketSerialWarrantyEnabled && ($coremarketOperationsOwner || auth()->user()?->can('warranty.claims.view'));
        $coremarketCanSuppliers = $coremarketPurchasingOperationsEnabled && ($coremarketOperationsOwner || auth()->user()?->can('suppliers.view'));
        $coremarketCanSupplierLedger = $coremarketPurchasingOperationsEnabled && ($coremarketOperationsOwner || auth()->user()?->can('supplier_ledger.view'));
        ...
        $coremarketCanPurchaseReceipts = $coremarketCanPurchaseOrders;
        $coremarketCanPurchaseReturns = $coremarketPurchasingOperationsEnabled && ($coremarketOperationsOwner || auth()->user()?->can('purchase_returns.view'));
        $coremarketCanPurchasing = $coremarketCanSuppliers || $coremarketCanPurchaseOrders || $coremarketCanPurchaseReceipts || $coremarketCanPurchaseReturns;
        ...
        $coremarketCanAccountingReports = $coremarketAccountingCoreEnabled && ($coremarketOperationsOwner || auth()->user()?->can('accounting_summary.view'));
        ...
        $coremarketBranchPricingEnabled = app(\App\Services\CoreMarketBranchPricingService::class)->branchPricingEnabled();
        $coremarketCanBranchPrices = $coremarketOperationsOwner
            || ($coremarketBranchPricingEnabled && auth()->user()?->can('pricing.branch_prices.view'));
        $coremarketCanDocumentTemplates = $coremarketOperationsOwner || auth()->user()?->can('document_templates.view');
        $coremarketCanPrintLabels = $coremarketOperationsOwner || auth()->user()?->can('document_templates.preview');
        $coremarketCanDeliveries = $coremarketOperationsOwner
            || auth()->user()?->can('deliveries.view')
            || auth()->user()?->can('deliveries.view_all')
            || auth()->user()?->can('deliveries.view_assigned');
        ...
        $coremarketCanLoyaltyMovements = $coremarketLoyaltyEnabled && ($coremarketOperationsOwner || auth()->user()?->can('loyalty.movements.view'));
        $coremarketCanLoyaltyAdjust = $coremarketLoyaltyEnabled && ($coremarketOperationsOwner || auth()->user()?->can('loyalty.adjust'));
        $coremarketCanLoyalty = $coremarketCanLoyaltyView || $coremarketCanLoyaltyRules || $coremarketCanLoyaltyMovements || $coremarketCanLoyaltyAdjust;
        $coremarketCanSalesOperations = $coremarketCanPos || $coremarketCanSalesReturns || $coremarketCanDeliveries;
        ...
        $coremarketCanInventory = $coremarketCanInventoryDashboard || $coremarketCanInventoryStock || $coremarketCanInventoryLookup || $coremarketCanInventoryLowStock || $coremarketCanInventoryMovements || $coremarketCanInventoryAudit || $coremarketCanInventoryPolicy || $coremarketCanInventoryFamilies || $coremarketCanInventoryAdjustments || $coremarketCanStockCounts || $coremarketCanBranchStock || $coremarketCanStockTransfers || $coremarketCanWarranty;
        $coremarketHasOperationsLinks = $coremarketCanOperationsOverview || $coremarketCanBranches || $coremarketCanInventory || $coremarketCanPurchasing || $coremarketCanSalesReturns || $coremarketCanDeliveries || $coremarketCanExpenses || $coremarketCanAccountingSummary || $coremarketCanAccountingReports || $coremarketCanAccountingCore || $coremarketCanCashboxes || $coremarketCanCashShifts || $coremarketCanCashMovements || $coremarketCanPos || $coremarketCanPriceLists || $coremarketCanBranchPrices || $coremarketCanDocumentTemplates || $coremarketCanPrintLabels || $coremarketCanLoyalty;
    ...
    <div class="aiz-sidebar left c-scrollbar">
        ...
        <div class="aiz-side-nav-wrap">
            ...
            <ul class="aiz-side-nav-list" id="main-menu" data-toggle="aiz-side-menu">
                ...
                <li class="aiz-side-nav-item">
                    <a href="#" class="aiz-side-nav-link {{ areActiveRoutes(['operations.overview', 'operations.deliveries.index', 'operations.deliveries.show', 'operations.inventory.dashboard', 'operations.inventory.stock', 'operations.inventory.barcode-lookup', 'operations.inventory.low-stock', 'operations.inventory.audit', 'operations.inventory.stock.adjust', 'operations.inventory.stock.adjust.store', 'operations.inventory.adjustments.index', 'operations.inventory.adjustments.create', 'operations.inventory.adjustments.show', 'operations.inventory.opening-stock.create', 'operations.inventory.stock-counts.index', 'operations.inventory.stock-counts.create', 'operations.inventory.stock-counts.show', 'operations.inventory.families.index', 'operations.inventory.families.create', 'operations.inventory.families.edit', 'operations.inventory-movements', 'operations.suppliers', 'operations.suppliers.create', 'operations.suppliers.show', 'operations.suppliers.edit', 'operations.purchase-orders', 'operations.purchase-orders.create', 'operations.purchase-orders.show', 'operations.purchase-receipts', 'operations.purchase-receipts.show', 'operations.purchase-returns', 'operations.purchase-returns.create', 'operations.purchase-returns.show', 'operations.sales-returns', 'operations.sales-returns.create', 'operations.sales-returns.show', 'operations.expenses', 'operations.expenses.create', 'operations.expenses.show', 'operations.accounting-summary', 'operations.accounting.reports', 'operations.accounting.dashboard', 'operations.accounting.core', 'operations.accounting.accounts', 'operations.accounting.accounts.show', 'operations.accounting.journals', 'operations.accounting.journals.show', 'operations.accounting.events', 'operations.accounting.general-ledger', 'operations.accounting.trial-balance', 'operations.accounting.profit-loss', 'operations.accounting.vat-snapshots', 'operations.accounting.vat-audit', 'operations.cashbox.dashboard', 'operations.cashboxes', 'operations.cashboxes.create', 'operations.cashboxes.show', 'operations.cashboxes.edit', 'operations.cash-shifts', 'operations.cash-shifts.show', 'operations.cash-shifts.open.form', 'operations.cash-movements.create', 'operations.cash-shifts.close.form', 'operations.cash-movements', 'operations.pos', 'operations.pos.search', 'operations.pos.receipt', 'operations.price-lists.index', 'operations.price-lists.create', 'operations.price-lists.show', 'operations.price-lists.edit', 'operations.branch-prices.index', 'operations.branch-prices.create', 'operations.branch-prices.edit', 'operations.document-templates.index', 'operations.document-templates.create', 'operations.document-templates.edit', 'operations.labels.index', 'operations.loyalty.dashboard', 'operations.loyalty.rules', 'operations.loyalty.accounts.index', 'operations.loyalty.accounts.show', 'operations.loyalty.movements.index', 'operations.loyalty.orders.show']) }}">
                        <div class="aiz-side-nav-icon">
                    ...
                    <ul class="aiz-side-nav-list level-2">
                        ...
                        @endif
                        @if($coremarketCanDocumentTemplates)<li class="aiz-side-nav-item"><a href="{{ route('operations.document-templates.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.document-templates.index', 'operations.document-templates.create', 'operations.document-templates.edit']) }}"><span class="aiz-side-nav-text">{{ translate('Document Templates') }}</span></a></li>@endif
                        @if($coremarketCanPrintLabels)<li class="aiz-side-nav-item"><a href="{{ route('operations.labels.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.labels.index']) }}"><span class="aiz-side-nav-text">{{ translate('Print Labels') }}</span></a></li>@endif
                        @if ($coremarketCanSalesOperations)
                        <li class="aiz-side-nav-item" data-coremarket-nav-group="sales">
                            <a href="#" class="aiz-side-nav-link {{ areActiveRoutes(['operations.pos', 'operations.pos.search', 'operations.pos.receipt', 'operations.sales-returns', 'operations.sales-returns.create', 'operations.sales-returns.show', 'operations.deliveries.index', 'operations.deliveries.show']) }}"><span class="aiz-side-nav-text">{{ translate('Sales') }}</span><span class="aiz-side-nav-arrow"></span></a>
                            <ul class="aiz-side-nav-list level-3">
                                @if ($coremarketCanPos)<li class="aiz-side-nav-item"><a href="{{ route('operations.pos') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.pos', 'operations.pos.search', 'operations.pos.receipt']) }}"><span class="aiz-side-nav-text">{{ translate('POS Sale') }}</span></a></li>@endif
                                @if ($coremarketCanSalesReturns)<li class="aiz-side-nav-item"><a href="{{ route('operations.sales-returns') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.sales-returns', 'operations.sales-returns.create', 'operations.sales-returns.show']) }}"><span class="aiz-side-nav-text">{{ translate('Sales Returns') }}</span></a></li>@endif
                                @if ($coremarketCanDeliveries)<li class="aiz-side-nav-item"><a href="{{ route('operations.deliveries.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.deliveries.index', 'operations.deliveries.show']) }}"><span class="aiz-side-nav-text">{{ translate('Delivery Board') }}</span></a></li>@endif
                            </ul>
                        </li>
                        @endif
                        @if ($coremarketCanInventory)
                        <li class="aiz-side-nav-item" data-coremarket-nav-group="inventory">
                            <a href="#" class="aiz-side-nav-link {{ areActiveRoutes(['operations.inventory.dashboard', 'operations.inventory.stock', 'operations.inventory.branch-stock', 'operations.inventory.stock-transfers.index', 'operations.inventory.stock-transfers.create', 'operations.inventory.stock-transfers.show', 'operations.inventory.barcode-lookup', 'operations.inventory.low-stock', 'operations.inventory.audit', 'operations.inventory.policy', 'operations.inventory.policy.update', 'operations.inventory.stock.adjust', 'operations.inventory.stock.adjust.store', 'operations.inventory.adjustments.index', 'operations.inventory.adjustments.create', 'operations.inventory.adjustments.show', 'operations.inventory.opening-stock.create', 'operations.inventory.stock-counts.index', 'operations.inventory.stock-counts.create', 'operations.inventory.stock-counts.show', 'operations.inventory.families.index', 'operations.inventory.families.create', 'operations.inventory.families.edit', 'operations.inventory-movements', 'operations.warranty.index', 'operations.warranty.claims.show']) }}"><span class="aiz-side-nav-text">{{ translate('Inventory') }}</span><span class="aiz-side-nav-arrow"></span></a>
                            <ul class="aiz-side-nav-list level-3">
                                ...
                                @if ($coremarketCanBranchStock)<li class="aiz-side-nav-item"><a href="{{ route('operations.inventory.branch-stock') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.inventory.branch-stock']) }}"><span class="aiz-side-nav-text">{{ translate('Branch Stock') }}</span></a></li>@endif
                                @if ($coremarketCanStockTransfers)<li class="aiz-side-nav-item"><a href="{{ route('operations.inventory.stock-transfers.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.inventory.stock-transfers.index', 'operations.inventory.stock-transfers.create', 'operations.inventory.stock-transfers.show']) }}"><span class="aiz-side-nav-text">{{ translate('Stock Transfers') }}</span></a></li>@endif
                                @if ($coremarketCanInventoryMovements)<li class="aiz-side-nav-item"><a href="{{ route('operations.inventory-movements') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.inventory-movements']) }}"><span class="aiz-side-nav-text">{{ translate('Stock Movements') }}</span></a></li>@endif
                                @if ($coremarketCanInventoryAdjustments)<li class="aiz-side-nav-item"><a href="{{ route('operations.inventory.adjustments.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.inventory.adjustments.index', 'operations.inventory.adjustments.create', 'operations.inventory.adjustments.show', 'operations.inventory.opening-stock.create']) }}"><span class="aiz-side-nav-text">{{ translate('Opening Stock / Adjustments') }}</span></a></li>@endif
                                @if ($coremarketCanStockCounts)<li class="aiz-side-nav-item"><a href="{{ route('operations.inventory.stock-counts.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.inventory.stock-counts.index', 'operations.inventory.stock-counts.create', 'operations.inventory.stock-counts.show']) }}"><span class="aiz-side-nav-text">{{ translate('Stock Counts') }}</span></a></li>@endif
                                @if ($coremarketCanInventoryFamilies)<li class="aiz-side-nav-item"><a href="{{ route('operations.inventory.families.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.inventory.families.index', 'operations.inventory.families.create', 'operations.inventory.families.edit']) }}"><span class="aiz-side-nav-text">{{ translate('Product Families') }}</span></a></li>@endif
                                @if ($coremarketCanWarranty)<li class="aiz-side-nav-item"><a href="{{ route('operations.warranty.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.warranty.index', 'operations.warranty.claims.show']) }}"><span class="aiz-side-nav-text">{{ translate('Warranty / Serials') }}</span></a></li>@endif
                                @if ($coremarketCanInventoryPolicy)<li class="aiz-side-nav-item"><a href="{{ route('operations.inventory.policy') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.inventory.policy', 'operations.inventory.policy.update']) }}"><span class="aiz-side-nav-text">{{ translate('Inventory Policy') }}</span></a></li>@endif
                        ...
                        @endif
                        @if ($coremarketCanPurchasing)
                        <li class="aiz-side-nav-item" data-coremarket-nav-group="purchasing">
                            <a href="#" class="aiz-side-nav-link {{ areActiveRoutes(['operations.suppliers', 'operations.suppliers.create', 'operations.suppliers.show', 'operations.suppliers.edit', 'operations.purchase-orders', 'operations.purchase-orders.create', 'operations.purchase-orders.show', 'operations.purchase-receipts', 'operations.purchase-receipts.show', 'operations.purchase-returns', 'operations.purchase-returns.create', 'operations.purchase-returns.show']) }}"><span class="aiz-side-nav-text">{{ translate('Purchasing') }}</span><span class="aiz-side-nav-arrow"></span></a>
                            <ul class="aiz-side-nav-list level-3">
                                @if ($coremarketCanSuppliers)<li class="aiz-side-nav-item"><a href="{{ route('operations.suppliers') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.suppliers', 'operations.suppliers.create', 'operations.suppliers.show', 'operations.suppliers.edit']) }}"><span class="aiz-side-nav-text">{{ translate('Suppliers') }}</span></a></li>@endif
                                @if ($coremarketCanPurchaseOrders)<li class="aiz-side-nav-item"><a href="{{ route('operations.purchase-orders') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.purchase-orders', 'operations.purchase-orders.create', 'operations.purchase-orders.show']) }}"><span class="aiz-side-nav-text">{{ translate('Purchase Orders') }}</span></a></li>@endif
                                @if ($coremarketCanPurchaseReceipts)<li class="aiz-side-nav-item"><a href="{{ route('operations.purchase-receipts') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.purchase-receipts', 'operations.purchase-receipts.show']) }}"><span class="aiz-side-nav-text">{{ translate('Purchase Receipts') }}</span></a></li>@endif
                                @if ($coremarketCanPurchaseReturns)<li class="aiz-side-nav-item"><a href="{{ route('operations.purchase-returns') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.purchase-returns', 'operations.purchase-returns.create', 'operations.purchase-returns.show']) }}"><span class="aiz-side-nav-text">{{ translate('Purchase Returns') }}</span></a></li>@endif
                            </ul>
                        </li>
                        @endif
                        @if ($coremarketCanPriceLists || $coremarketCanBranchPrices)
                        <li class="aiz-side-nav-item" data-coremarket-nav-group="pricing">
                            <a href="#" class="aiz-side-nav-link {{ areActiveRoutes(['operations.price-lists.index', 'operations.price-lists.create', 'operations.price-lists.show', 'operations.price-lists.edit', 'operations.branch-prices.index', 'operations.branch-prices.create', 'operations.branch-prices.edit']) }}"><span class="aiz-side-nav-text">{{ translate('Pricing') }}</span><span class="aiz-side-nav-arrow"></span></a>
                            <ul class="aiz-side-nav-list level-3">
                                @if($coremarketCanPriceLists)<li class="aiz-side-nav-item"><a href="{{ route('operations.price-lists.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.price-lists.index', 'operations.price-lists.create', 'operations.price-lists.show', 'operations.price-lists.edit']) }}"><span class="aiz-side-nav-text">{{ translate('Price Lists') }}</span></a></li>@endif
                                @if($coremarketCanBranchPrices)<li class="aiz-side-nav-item"><a href="{{ route('operations.branch-prices.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.branch-prices.index', 'operations.branch-prices.create', 'operations.branch-prices.edit']) }}"><span class="aiz-side-nav-text">{{ translate('Branch Prices') }}</span></a></li>@endif
                            </ul>
                        </li>
                        @endif
                        @if ($coremarketCanAccountingOperations)
                        <li class="aiz-side-nav-item" data-coremarket-nav-group="accounting">
                            <a href="#" class="aiz-side-nav-link {{ areActiveRoutes(['operations.expenses', 'operations.expenses.create', 'operations.expenses.show', 'operations.cashbox.dashboard', 'operations.cashboxes', 'operations.cashboxes.create', 'operations.cashboxes.show', 'operations.cashboxes.edit', 'operations.cash-shifts', 'operations.cash-shifts.show', 'operations.cash-shifts.open.form', 'operations.cash-movements.create', 'operations.cash-shifts.close.form', 'operations.cash-movements', 'operations.accounting-summary', 'operations.accounting.reports', 'operations.accounting.dashboard', 'operations.accounting.core', 'operations.accounting.accounts', 'operations.accounting.accounts.show', 'operations.accounting.journals', 'operations.accounting.journals.show', 'operations.accounting.events', 'operations.accounting.general-ledger', 'operations.accounting.trial-balance', 'operations.accounting.profit-loss', 'operations.accounting.vat-snapshots', 'operations.accounting.vat-audit']) }}"><span class="aiz-side-nav-text">{{ translate('Accounting') }}</span><span class="aiz-side-nav-arrow"></span></a>
                            <ul class="aiz-side-nav-list level-3">
                                @if ($coremarketCanExpenses)<li class="aiz-side-nav-item"><a href="{{ route('operations.expenses') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.expenses', 'operations.expenses.create', 'operations.expenses.show']) }}"><span class="aiz-side-nav-text">{{ translate('Expenses') }}</span></a></li>@endif
                                @if ($coremarketCanCashboxes)<li class="aiz-side-nav-item"><a href="{{ route('operations.cashbox.dashboard') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.cashbox.dashboard', 'operations.cashboxes', 'operations.cashboxes.create', 'operations.cashboxes.show', 'operations.cashboxes.edit', 'operations.cash-shifts.open.form']) }}"><span class="aiz-side-nav-text">{{ translate('Cashbox') }}</span></a></li>@endif
                                @if ($coremarketCanCashShifts)<li class="aiz-side-nav-item"><a href="{{ route('operations.cash-shifts') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.cash-shifts', 'operations.cash-shifts.show', 'operations.cash-shifts.close.form', 'operations.cash-movements.create']) }}"><span class="aiz-side-nav-text">{{ translate('Cashbox Shifts') }}</span></a></li>@endif
                                ...
                                @if ($coremarketCanAccountingSummary)<li class="aiz-side-nav-item"><a href="{{ route('operations.accounting-summary') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.accounting-summary']) }}"><span class="aiz-side-nav-text">{{ translate('Accounting Summary') }}</span></a></li>@endif
                                @if ($coremarketCanAccountingReports)<li class="aiz-side-nav-item"><a href="{{ route('operations.accounting.reports') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.accounting.reports']) }}"><span class="aiz-side-nav-text">{{ translate('Accounting Reports') }}</span></a></li>@endif
                                @if ($coremarketCanAccountingCore)<li class="aiz-side-nav-item"><a href="{{route('operations.accounting.dashboard')}}" class="aiz-side-nav-link {{areActiveRoutes(['operations.accounting.dashboard','operations.accounting.core'])}}"><span class="aiz-side-nav-text">{{translate('Accounting Dashboard')}}</span></a></li>@endif
                        ...
                        @endif
                        @if ($coremarketCanLoyalty)
                        <li class="aiz-side-nav-item">
                            <a href="#" class="aiz-side-nav-link {{ areActiveRoutes(['operations.loyalty.dashboard', 'operations.loyalty.rules', 'operations.loyalty.accounts.index', 'operations.loyalty.accounts.show', 'operations.loyalty.movements.index', 'operations.loyalty.orders.show']) }}"><span class="aiz-side-nav-text">{{ translate('Loyalty') }}</span><span class="aiz-side-nav-arrow"></span></a>
                            <ul class="aiz-side-nav-list level-3">
                                @if ($coremarketCanLoyaltyView)
                                <li class="aiz-side-nav-item"><a href="{{ route('operations.loyalty.dashboard') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.loyalty.dashboard']) }}"><span class="aiz-side-nav-text">{{ translate('Dashboard') }}</span></a></li>
                                <li class="aiz-side-nav-item"><a href="{{ route('operations.loyalty.accounts.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.loyalty.accounts.index', 'operations.loyalty.accounts.show']) }}"><span class="aiz-side-nav-text">{{ translate('Customer Balances') }}</span></a></li>
                                @endif
                                @if ($coremarketCanLoyaltyRules)<li class="aiz-side-nav-item"><a href="{{ route('operations.loyalty.rules') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.loyalty.rules']) }}"><span class="aiz-side-nav-text">{{ translate('Rules') }}</span></a></li>@endif
                                @if ($coremarketCanLoyaltyMovements)<li class="aiz-side-nav-item"><a href="{{ route('operations.loyalty.movements.index') }}" class="aiz-side-nav-link {{ areActiveRoutes(['operations.loyalty.movements.index']) }}"><span class="aiz-side-nav-text">{{ translate('Movements') }}</span></a></li>@endif
                            </ul>
                        </li>
                        @endif
                    </ul>
                ...
                'view_all_brands', 'brand_bulk_upload','view_product_attributes','view_colors','view_product_reviews'])
                <li class="aiz-side-nav-item">
                    <a href="#" class="aiz-side-nav-link">
                        <div class="aiz-side-nav-icon">
                    ...
                    <ul class="aiz-side-nav-list level-2">
                        ...
                        <li class="aiz-side-nav-item">
                            ...
                            <ul class="aiz-side-nav-list level-3 {{ app()->getLocale() === 'ar' ? 'text-right' : '' }}">
                                <li class="aiz-side-nav-item">
                            ...
                            <style>
                                ...
                                body[dir='rtl'] .aiz-side-nav-list {
                                    list-style-position: inside;
                                    padding-right: 0;
                                }
                ...
                <li class="aiz-side-nav-item">
                    ...
                    <ul class="aiz-side-nav-list level-2">
                        ...
                        @can('seller_payment_history')
                        <li class="aiz-side-nav-item">
                            <a href="{{ route('sellers.requests.index') }}" class="aiz-side-nav-link">
                                <i class="las la-folder-open aiz-side-nav-icon"></i>
                                <span class="aiz-side-nav-text">{{ translate('Requests for open brand or category')
                                    }}</span>
                            </a>
                        </li>
                        @endcan
                            ...
                            <ul class="aiz-side-nav-list level-3">
                                ...
                                <li class="aiz-side-nav-item">
                                    <a href="{{ route('custom_notification.history') }}" class="aiz-side-nav-link">
                                        <span class="aiz-side-nav-text">{{translate('Custom Notification
                                            History')}}</span>
                                    </a>
                                </li>
                                @endcan
                ...
                <li class="aiz-side-nav-item">
                    ...
                    <ul class="aiz-side-nav-list level-2">
                        ...
                        Auth::user()->id)->where('receiver_viewed', '0')->get();
                        @endphp
                        <li class="aiz-side-nav-item">
