...
<div class="card"><div class="card-body">
    ...
    @forelse($purchaseOrders as $order)@php($ordered = (float) $order->items->sum('quantity_ordered'))@php($received = (float) $order->items->sum('quantity_received'))<tr>
        <td>{{ $order->purchase_number }}</td><td>{{ $order->supplier?->name ?: '-' }}</td><td><span class="badge badge-info">{{ translate(ucfirst(str_replace('_', ' ', $order->status))) }}</span></td><td>{{ optional($order->ordered_at)->format('Y-m-d') ?: '-' }}</td><td>{{ coremarket_quantity($received) }} / {{ coremarket_quantity($ordered) }}</td><td>{{ coremarket_money($order->total_amount, $order->currency) }}</td><td class="text-right"><a class="btn btn-soft-primary btn-sm" href="{{ route('operations.purchase-orders.show', $order) }}">{{ translate('View') }}</a></td>
