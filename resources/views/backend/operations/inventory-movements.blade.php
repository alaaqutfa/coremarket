...
<div class="card"><div class="card-body">
    <form class="row gutters-5 mb-3">
        ...
        <div class="col-md-2"><select class="form-control" name="movement_type"><option value="">{{ translate('All types') }}</option>@foreach(['sale','sale_reversal','purchase','purchase_return','opening_stock','stock_adjustment','stock_count_variance','emergency_adjustment','reservation','release'] as $type)<option value="{{ $type }}" @selected(request('movement_type')===$type)>{{ $type }}</option>@endforeach</select></div>
    ...
    <div class="table-responsive"><table class="table aiz-table">
        ...
        @forelse($movements as $movement)
            <tr>
                ...
                <td>
                    {{ $movement->metadata['document_reference'] ?? class_basename($movement->reference_type) }}
