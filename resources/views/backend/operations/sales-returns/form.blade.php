...
<div class="card"><div class="card-body">
    ...
    <form method="POST" action="{{ route('operations.sales-returns.store') }}">@csrf
        ...
        <div class="table-responsive"><table class="table table-bordered"><thead><tr><th>{{ translate('Product') }}</th><th>{{ translate('Variant / SKU / Barcode') }}</th><th>{{ translate('Sold') }}</th><th>{{ translate('Previously Returned') }}</th><th>{{ translate('Remaining') }}</th><th>{{ translate('Unit Price') }}</th><th>{{ translate('Cost / Profit') }}</th><th>{{ translate('Return quantity') }}</th><th>{{ translate('Serial / IMEI') }}</th><th>{{ translate('Reason') }}</th></tr></thead><tbody>
        @foreach($returnableRows as $row)@php($detail = $row['detail'])@php($stock = $row['product_stock'])<tr><td>{{ $detail->product?->name ?: '#'.$detail->product_id }}</td><td>{{ $detail->variation ?: '-' }}<br><small>{{ $stock?->sku ?: '-' }} / {{ $stock?->barcode ?: $detail->product?->barcode ?: '-' }}</small></td><td>{{ coremarket_quantity($detail->quantity) }}</td><td>{{ coremarket_quantity($row['returned_quantity']) }}</td><td>{{ coremarket_quantity($row['remaining_quantity']) }}</td><td>{{ coremarket_money($row['unit_price']) }}</td><td>@if($row['unknown_cost'])<span class="text-warning">{{ translate('Cost snapshot unavailable') }}</span>@else{{ coremarket_money($detail->cost_price) }} / {{ coremarket_money($detail->profit_amount) }}@endif</td><td><input class="form-control" type="number" step="0.000001" min="0" max="{{ $row['remaining_quantity'] }}" name="items[{{ $loop->index }}][quantity]" value="0" @disabled($row['remaining_quantity'] <= 0)><input type="hidden" name="items[{{ $loop->index }}][order_detail_id]" value="{{ $detail->id }}"></td><td>@forelse($row['serial_units'] as $unit)<label class="d-block mb-1"><input type="checkbox" name="items[{{ $loop->parent->index }}][serial_unit_ids][]" value="{{ $unit->id }}"> {{ $unit->serial_number ?: $unit->imei_1 }}</label>@empty<span class="text-muted">-</span>@endforelse</td><td><input class="form-control" name="items[{{ $loop->index }}][reason]" @disabled($row['remaining_quantity'] <= 0)></td></tr>@endforeach
        </tbody></table></div>
