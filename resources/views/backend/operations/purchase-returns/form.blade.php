...
<div class="card"><div class="card-body">
    ...
    <form method="POST" action="{{ route('operations.purchase-returns.store') }}">@csrf
        ...
        @if($branchInventoryEnabled)<div class="form-group"><label>{{ translate('Stock Branch') }}</label><select class="form-control" name="branch_id" required>@foreach($branches as $branch)<option value="{{ $branch->id }}">{{ $branch->name }}{{ $branch->is_default ? ' ('.translate('Default').')' : '' }}</option>@endforeach</select></div>@endif
