                ...
                <div class="col-md-2 ml-auto">
                    <select class="form-control form-control-sm aiz-selectpicker mb-2 mb-md-0" name="type" id="type"
                        ...
                        <option value="rating,desc"
                            @isset($col_name) @isset($query) @if ($col_name == 'rating' && $query == 'desc') selected @endif      @endisset
                        @endisset>{{ translate('Rating (High > Low)') }}</option>
                    <option value="rating,asc"
                        @isset($col_name) @isset($query) @if ($col_name == 'rating' && $query == 'asc') selected @endif       @endisset
                    @endisset>{{ translate('Rating (Low > High)') }}</option>
                <option
                    value="num_of_sale,desc"@isset($col_name) @isset($query) @if ($col_name == 'num_of_sale' && $query == 'desc') selected @endif @endisset
                @endisset>{{ translate('Num of Sale (High > Low)') }}</option>
            <option value="num_of_sale,asc"
                @isset($col_name) @isset($query) @if ($col_name == 'num_of_sale' && $query == 'asc') selected @endif      @endisset
            @endisset>{{ translate('Num of Sale (Low > High)') }}</option>
        <option value="unit_price,desc"
            @isset($col_name) @isset($query) @if ($col_name == 'unit_price' && $query == 'desc') selected @endif  @endisset
        @endisset>{{ translate('Base Price (High > Low)') }}</option>
    <option value="unit_price,asc"
        @isset($col_name) @isset($query) @if ($col_name == 'unit_price' && $query == 'asc') selected @endif   @endisset
